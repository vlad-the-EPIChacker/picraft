...
def floor(x, z, w, len,i):
    ...
    mc.setBlocks(x+w/2,h/2+hi,z,x+w/2+1,h/2+1+hi,z,glass)#window#  z
    mc.setBlocks(x,h/2+hi,z+len/2,x,h/2+1+hi,z+len/2+1,glass)#window# x
    
    
    if i==0:
        mc.setBlock(x+w/2,h/2+hi-1,z+len,door,6)#door#
        mc.setBlock(x+w/2,h/2+hi,z+len,door,6)#door#
    else:       
        mc.setBlocks(x+w/2+1,h/2+hi,z+len,x+w/2,h/2+1+hi,z+len,glass)#window# z
        
    mc.setBlocks(x+w,h/2+hi,z+len/2,x+w,h/2+1+hi,z+len/2+1,glass)#window# x
def building(x,z,f):
    w=12
    len=14
    for i in range(0,f):
